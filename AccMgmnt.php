	...
	public function login($data)
	{
		if(isset($data['appid']) && $data['appid'] == $this->config->app_id)
		{
			if(isset($data['username']) && isset($data['password']))
			{
				...
				$query = odbc_exec($this->config->con,"SELECT c_headera FROM account WHERE c_id='$username'");
	...
	public function register($data)
	{
		if(isset($data['appid']) && $data['appid'] == $this->config->app_id)
		{
			if(isset($data['username']) && isset($data['password']) && isset($data['email']))
			{
				...
				$password = $data['password'];
				$email = $data['email'];
				$query = odbc_exec($this->config->con,"SELECT * FROM account WHERE c_id='$username'");
				$num_rows = odbc_num_rows($query);
				if($num_rows == 0)
				{
					$date = date();
					...
					$query1 = odbc_exec($this->config->con, $sql1);
					if(!$query1)
    ...
    */
	public function forgot_passwd($data)
	{
		if(isset($data['appid']) && $data['appid'] == $this->config->app_id)
		{
			...
			{
				$username = $data['username'];
				$query = odbc_exec($this->config->con,"SELECT c_headerb FROM account WHERE c_id='$username'");
				$num_rows = odbc_num_rows($query);
				if($num_rows != 0)
				{
